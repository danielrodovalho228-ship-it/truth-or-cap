  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      if (Date.now() - startedAtRef.current > TIMEOUT_MS) {
        ...
        setError('Análise demorando mais que o esperado. Atualiza a página.');
      }
    }, POLL_INTERVAL_MS);
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 max-w-md mx-auto w-full text-center relative">
        ...
        <h1 className="font-display text-5xl md:text-6xl font-black leading-[0.9] tracking-tight mb-4">
          Não<br />
          <span className="italic font-light">vacila.</span>
        </h1>

        <p className="font-body text-sm text-fg-muted leading-relaxed mb-10 max-w-sm">
          {mode === 'text'
            ? 'Claude está lendo seu texto procurando pistas. Geralmente em menos de 10s.'
            : 'Quatro modelos de IA inspecionando sua evidência. Tempo médio 15-30s.'}
        </p>
        ...
        <p className="font-mono text-[10px] tracking-widest uppercase text-fg-muted">
          {Math.floor(elapsed / 1000)}s decorridos
        </p>
      ...
      {revealSus !== null ? (
        <DramaticReveal
          susLevel={revealSus}
          lang={lang}
          onComplete={() => router.replace(`/jogo/${gameId}`)}
        />
      ) : null}
    </main>
  );
