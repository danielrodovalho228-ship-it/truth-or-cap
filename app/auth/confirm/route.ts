 ...
 * confirmations. Supabase sends a token_hash + type, NOT a code, so we use
...
export async function GET(request: NextRequest) {
  ...
  if (!token_hash || !type) {
    ...
    url.searchParams.set('error', 'Missing confirmation token');
  ...
  if (error) {
    ...
    const url = new URL('/auth/sign-in', baseUrl);
    url.searchParams.set('error', 'Confirmation expired or invalid');
    return NextResponse.redirect(url);
  }
